...
class interpreter {
    ...
    visit(node) {
        ...
        if (nodeName == "addNode") {
            return new numberValue(this.visit(node.nodeA).value + this.visit(node.nodeB).value);
        }
            ...
            try {
                let value = this.divideIfNotZero(this.visit(node.nodeA).value, this.visit(node.nodeB).value);
